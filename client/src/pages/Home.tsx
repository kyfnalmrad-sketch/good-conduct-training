...
function Barcode({ value }: { value: string }) { const ref = useRef<SVGSVGElement>(null); useEffect(() => { if (ref.current) JsBarcode(ref.current, value || "TRAINING", { format: "CODE128", displayValue: false, margin: 0, width: 1.25, height: 24 }); }, [value]); return <svg ref={ref} className="linear-barcode" aria-label="باركود شريطي" />; }
...
function DocumentPreview({ data, photo }: { data: FormState; photo: string }) {
  ...
  return <div className="document-wrap"><article className="document" id="print-document"><div className="word-template-bg" style={{ backgroundImage: `url(${wordTemplate})` }} />
    <div className="doc-top"><div className="photo-stack"><img className="doc-photo" src={photo} alt="الصورة الشخصية" /><Barcode value={`${data.referenceNo}|${data.internalNo}`} /></div><div className="doc-meta"><div><small>Issue Date | تاريخ الإصدار</small><b>{data.issueDate}</b></div><div><small>No. | رقم القيد</small><b>{data.issueNo}</b></div></div><div className="qr-box"><QRCodeSVG value={JSON.stringify({ reference: data.referenceNo, internal: data.internalNo })} size={96} level="H" includeMargin /><span>QR</span></div></div>
