...
function DocumentPreview({ data, photo }: { data: FormState; photo: string }) {
  ...
  return <div className="document-wrap"><article className="document" id="print-document"><div className="word-template-bg" style={{ backgroundImage: `url(${wordTemplate})` }} />
    <div className="doc-top"><img className="doc-photo" src={photo} alt="الصورة الشخصية" /><div className="doc-meta"><div><small>Issue Date | تاريخ الإصدار</small><b>{data.issueDate}</b></div><div><small>No. | رقم القيد</small><b>{data.issueNo}</b></div></div><div className="qr-box"><QRCodeSVG value={JSON.stringify({ reference: data.referenceNo, internal: data.internalNo, issue: data.issueNo, name: data.fullNameEn, id: data.idNumberEn })} size={64} level="M" includeMargin /><span>QR</span></div></div><div className="doc-identifiers"><span>Reference | المرجعي: <b>{data.referenceNo}</b></span><span>Internal | الداخلي: <b>{data.internalNo}</b></span><div className="barcode-box"><Barcode value={data.referenceNo} /><small>BARCODE</small></div></div>
