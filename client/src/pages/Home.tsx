...
type FormState = {
  issueNo: string; referenceNo: string; internalNo: string; issueDate: string;
...
const initial: FormState = {
  issueNo: "20059", referenceNo: "20260903-1811", internalNo: "INT-00020059", issueDate: "11/12/2025",
...
function DocumentPreview({ data, photo }: { data: FormState; photo: string }) {
  ...
  const payload = JSON.stringify({ reference: data.referenceNo, internal: data.internalNo, issueNo: data.issueNo });
  return <div className="document-wrap"><article className="document" id="print-document">
    ...
    <div className="doc-top"><div className="photo-stack"><img className="doc-photo" src={photo} alt="الصورة الشخصية" /><Barcode value={`${data.referenceNo}|${data.internalNo}`} /></div><div className="doc-meta"><div><small>Issue Date | تاريخ الإصدار</small><b>{data.issueDate}</b></div><div><small>No. | رقم القيد</small><b>{data.issueNo}</b></div></div><div className="qr-box"><QRCodeSVG value={payload} size={96} level="H" includeMargin /><span>جعل جرائم</span></div></div>
