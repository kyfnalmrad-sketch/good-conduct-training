...
export function LetterPreview({ data, language }: { data: WorkLetterData; language: Language }) {
  ...
  return <article className={`work-letter-paper ${english ? "is-english" : "is-arabic"}`} dir={english ? "ltr" : "rtl"}>
    ...
    <div className="letter-signature"><strong>{english ? "Human Resources Department" : "إدارة الموارد البشرية"}</strong><span>{english ? "Issued by:" : "صادر من:"}</span><b>{english ? "Ahmed Mohammed, Human Resources Manager" : "أحمد محمد، مدير الموارد البشرية"}</b></div>
